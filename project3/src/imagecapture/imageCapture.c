...
double colAngThresh=.725;		// Colour angle threshold
...
int imageCaptureStartup(char *devName, int rx, int ry, int own_col, int AI_mode)
{
 ...
 //  - Initialize the webcam and open the video input device
 //  - Set up and open an OpenGL window for image display
 //  - Initialize the AI data structure
 //  - Call the image processing loop (which will run until the program 
 //    terminates)
 //  
 ///////////////////////////////////////////////////////////////////////////////////
 toggleProc=0;
  ...
  ***************************************************/
  ...
  if (toggleProc==1)
  {
   ...
   if (f!=NULL)
   {
    ...
    gotbg=1;
    cornerIdx=4;
    fprintf(stderr,"Successfully read calibration data from file\n");
   }
   else
  ...
  if (H==NULL)
  {
   ...
   dx=(double)sx/1023.0;
   dy=(double)sy/767.0;
#pragma omp parallel for schedule(dynamic,16) private(ii,jj,i,j,dx,dy)
   for (j=0;j<768;j++)
    for (i=0;i<1024;i++)
    {
     ii=i*dx;
