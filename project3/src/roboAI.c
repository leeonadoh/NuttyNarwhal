...
void track_agents(struct RoboAI *ai, struct blob *blobs)
{
 ...
 if (p)
 {
  ...
  // Predict the ball's next position by adding velocity to previous position.
  // Ignore noisy velocity values.
  if (ai->st.bvx*ai->st.bvx + ai->st.bvy*ai->st.bvy > 25){
    ai->st.bcx = ai->st.old_bcx + 2*ai->st.bvx;
...
void soccerSM(struct RoboAI *ai){
  ...
  switch(ai->st.state){
    ...
    case 13: // Emergency defense. Move between ball and self goal.
      findQ(ai, qx, qy, Q_DIST);
      findDefendPoint(ai, Rx, Ry, OP_RADIUS, R_DIST);
      urgentX = (ai->st.bcx + (ai->st.side ? CAM_WIDTH : 0))/2;
      urgentY = (ai->st.bcy + CAM_HEIGHT/2)/2;
...
inline void soccerSMTrans(struct RoboAI *ai, double *Rx, double *Ry, double *qx, double*qy){
  switch(ai->st.state){
    ...
    case 4: // Attack, chase ball kick
      if (!ai->st.selfID) ai->st.state = 99;
      else if (ssmTransN(ai, 4)) ai->st.state = 2; //FIXED: Kick failed
    ...
    case 98: // Ball missing
      ...
      else if (ssmTransC(ai->st.old_scx, ai->st.old_scy)) ai->st.state = 99;
    break;
...
inline int ssmTransC(double px, double py){
  return (px < CLOSE_DIST || (CAM_WIDTH - px) < CLOSE_DIST) || 
         (py < CLOSE_DIST || (CAM_HEIGHT - py) < CLOSE_DIST);
}
...
inline int ssmTransI(struct RoboAI *ai, double *qx, double *qy){
  return !attackMode(ai) && 
    hasClearPath(OP_RADIUS, 
      (ai->st.side ? 0 : CAM_WIDTH), CAM_HEIGHT / 2, 
      ai->st.old_ocx, ai->st.old_ocy,
      ai->st.bcx, ai->st.bcy) && !pointObstructed(ai, CLOSE_DIST_MORE, *qx, *qy);
}

// If the ball is within a set radius of the bot
inline int ssmTransJ(struct RoboAI *ai){
  double dx = ai->st.old_scx - ai->st.bcx;
  double dy = ai->st.old_scy - ai->st.bcy;
...
inline int ssmTransN(struct RoboAI *ai, double multiple){
  double dx = ai->st.old_scx - ai->st.bcx;
  double dy = ai->st.old_scy - ai->st.bcy;
  return dx*dx + dy*dy > multiple*CLOSE_DIST*CLOSE_DIST;
}
...
void moveInDirection(struct RoboAI *ai, double x, double y, int pivot, int minSpeed){
  ...
  int rotSpeed = (angle > 0) ? (int)(15 + angle/4) : (int)(-15 + angle/4);
...
int attackMode(struct RoboAI *ai){
  ...
  double vToBallx = ai->st.bcx - ai->st.old_ocx;
  double vToBally = ai->st.bcy - ai->st.old_ocy;
  return vToGoalx*vToBallx + vToGoaly*vToBallx > 0;
}

/**
* Return whether point p is covered b3y opponent.
*/
...
void findQ(struct RoboAI *ai, double *qx, double *qy, int backoffDist){
  double vx = ai->st.bcx - (ai->st.side ? 0 : CAM_WIDTH);
  double vy = ai->st.bcy - CAM_HEIGHT / 2;
  double vmag = sqrt(vx*vx + vy*vy);
  *qx = ai->st.bcx + backoffDist*vx/vmag;
  *qy = ai->st.bcy + backoffDist*vy/vmag;
}
...
void findDefendPoint(struct RoboAI *ai, double *rx, double *ry, int size, int backoffDist){
  ...
  *rx = ai->st.bcx - backoffDist/rMag*(*rx);
  *ry = ai->st.bcy - backoffDist/rMag*(*ry);
  //printf("rx: %f, ry: %f\n", *rx, *ry);
}
